...
namespace DAALGOFinal.Traversals
{
    ...
    public partial class BFS : UserControl
    {
        bool stopcreate;
        int[,] matrix;
        int countnodes;
        ...
        public void bfsTraversal()
        {
            ...
            visited[source] = true; //Add start to visited
            Q.Enqueue(source); //Push start to Queue

            Thread.Sleep(200);

            while (Q.Count > 0) //if queue is not empty
            {
                int x = Convert.ToInt32(Q.Dequeue().ToString()); //Holds current node
                Thread.Sleep(500);
                ...
                {
                    Thread.Sleep(200);
                    if (visited[i] != true) //if adjacent is not visited
                    {
                        Q.Enqueue(i);
                        visited[x] = true;
                    }
